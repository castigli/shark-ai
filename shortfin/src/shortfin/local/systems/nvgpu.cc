...
namespace {
const std::string_view SYSTEM_DEVICE_CLASS = "nvgpu";
const std::string_view LOGICAL_DEVICE_CLASS = "gpu";
const std::string_view HAL_DRIVER_PREFIX = "cuda";
}  // namespace

NVGPUSystemBuilder::NVGPUSystemBuilder(iree_allocator_t host_allocator,
...
NVGPUSystemBuilder::~NVGPUSystemBuilder() = default;

void NVGPUSystemBuilder::InitializeDefaultSettings() {
//   // Library search path.
//   std::optional<std::string_view> search_path =
//       config_options().GetOption("nvgpu_cuda_dylib_path");
//   if (!search_path) {
//     // Fall back to the raw "IREE_CUDA_DYLIB_PATH" for compatibility with IREE
//     // tools.
//     search_path = config_options().GetRawEnv("IREE_CUDA_DYLIB_PATH");
//   }
//   if (search_path) {
//     for (auto entry : config_options().Split(*search_path, ';')) {
//       cuda_lib_search_paths_.push_back(std::string(entry));
//     }
//   }

  // Gets allocator specs from either "nvgpu_allocators" or the fallback
  // "allocators".
  nvgpu_allocator_specs_ = GetConfigAllocatorSpecs("nvgpu_allocators");
  ...
  auto tracing_level =
      config_options().GetInt("nvgpu_tracing_level", /*non_negative=*/true);
  ...
  auto logical_devices_per_physical_device = config_options().GetInt(
      "nvgpu_logical_devices_per_physical_device", /*non_negative=*/true);
  if (logical_devices_per_physical_device) {
    logical_devices_per_physical_device_ = *logical_devices_per_physical_device;
  }

  // CPU devices.
  cpu_devices_enabled_ = config_options().GetBool("nvgpu_cpu_devices_enabled");

  // Visible devices.
  std::optional<std::string_view> visible_devices_option =
      config_options().GetOption("nvgpu_visible_devices");
...
void NVGPUSystemBuilder::Enumerate() {
  if (cuda_hal_driver_) return;
  SHORTFIN_TRACE_SCOPE_NAMED("NVGPUSystemBuilder::Enumerate");
  ...
  for (iree_host_size_t i = 0; i < available_devices_count_; ++i) {
    ...
    logging::debug("Enumerated available NVGPU device: {} ({})",
                   to_string_view(info->path), to_string_view(info->name));
  }
}

std::vector<std::string> NVGPUSystemBuilder::GetAvailableDeviceIds() {
  Enumerate();
...
SystemPtr NVGPUSystemBuilder::CreateSystem() {
  SHORTFIN_TRACE_SCOPE_NAMED("NVGPUSystemBuilder::CreateSystem");
  ...
  if (available_devices_count_ == 0 &&
    ...
    throw std::invalid_argument("No NVGPU devices found/visible");
  }
